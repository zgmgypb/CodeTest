...
void *capture_open (const char *dev_name, int t_width, int t_height)
{
	...
	for (i = 0; i < sizeof(ctx->bufs)/sizeof(Buffer); i++) {
		struct v4l2_buffer buf;
		memset(&buf, 0, sizeof(buf));
...
int capture_get_picture (void *id, unsigned char **pbuf, int *pbuflen)
{
	// 获取, 转换
	Ctx *ctx = (Ctx*)id;
	struct v4l2_buffer buf;
...
{
	void *capture = capture_open("/dev/video0", 1280, 720);
	int i = 0;
	unsigned char *pbuf;
	int buflen;
	...
	for (i = 0; i < 500; i++) {
		...
		capture_get_picture(capture, &pbuf, &buflen);
