...
#include <assert.h>
#include <stdint.h>
#include <sys/types.h>		//size_t°¢pid_t°¢int8_t
...
void *SAMPLE_COMM_AUDIO_AencProc(void *parg)
{
    ...
    fd_set read_fds;
    struct timeval TimeoutVal;
	long long Star_time=0;
	long long End_time=0;
	static long long Max_time=0;
	static long long Min_time=5000;
	long long Tmp_time;
	HI_U32 Count_Achn=0;
	Count_Achn=pstAencCtl->AeChn;
...
* function : Start Ai
******************************************************************************/
HI_S32 AUDIO_StartAi(AUDIO_DEV AiDevId, HI_S32 s32AiChnCnt,  AIO_ATTR_S* pstAioAttr)							 
{
	HI_S32 i;
	HI_S32 s32Ret;
	AI_CHN_PARAM_S stAiChnPara;
...
* function : Stop Ai
******************************************************************************/
HI_S32 AUDIO_StopAi(AUDIO_DEV AiDevId, HI_S32 s32AiChnCnt)
{
...
HI_S32 AI_StartDev(AUDIO_DEV AiDevId, AIO_ATTR_S* pstAioAttr)
{
	HI_S32 s32Ret;
	...
	system("himm 0x13140140 0x64a9cd > /dev/null"); /* …Ë÷√ I2S  ‰≥ˆ ±÷” 24.576MHz */

	return HI_SUCCESS;
}

HI_S32 AI_StopDev(AUDIO_DEV AiDevId)
{
	HI_S32 s32Ret;
...
HI_S32 AI_StartChn(AUDIO_DEV AiDevId, AI_CHN ChnIndex)
{
	HI_S32 s32Ret;
	AI_CHN_PARAM_S stAiChnPara;

	s32Ret = HI_MPI_AI_EnableChn(AiDevId, ChnIndex);
	if (s32Ret) {
		printf("%s: HI_MPI_AI_EnableChn(%d,%d) failed with %#x\n", __FUNCTION__, AiDevId, ChnIndex, s32Ret);
		return s32Ret;
	}
	s32Ret = HI_MPI_AI_GetChnParam(AiDevId, ChnIndex, &stAiChnPara);
	if (HI_SUCCESS != s32Ret) {
		printf("%s: Get ai chn param failed\n", __FUNCTION__);
		return s32Ret;
	}
	...
	s32Ret = HI_MPI_AI_SetChnParam(AiDevId, ChnIndex, &stAiChnPara);
	if (HI_SUCCESS != s32Ret) {
		printf("%s: set ai chn param failed\n", __FUNCTION__);
		return s32Ret;
	}
...
HI_S32 AI_StopChn(AUDIO_DEV AiDevId, AI_CHN ChnIndex)
{
	HI_S32 s32Ret;

	s32Ret = HI_MPI_AI_DisableChn(AiDevId, ChnIndex);
	if (HI_SUCCESS != s32Ret) {
		printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "failed");
		return s32Ret;
	}
