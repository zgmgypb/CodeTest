...
void HWL_EncPsiCreateSdt(PSI_SdtInfo *pSdtInfo, PSI_PacketInfo *pPsiInfo)
{
	...
	if (lIndex > MPEG2_TS_PACKET_SIZE)
	{ /* SDT分多个包 */
		pPsiInfo->m_SdtPacketNum = 2;
		pPsiInfo->m_SecondSdtPacketPosition = pPsiInfo->m_PsiPacketCounter;
	}
	else
	{	
		pPsiInfo->m_SdtPacketNum = 1;
		pPsiInfo->m_SecondSdtPacketPosition = 0;
	}
...
/* 设置编码PSI表到主板 */
#define PSI_INSERT_INTERVAL 100
BOOL PSI_SetEncPsiParamToHw(PSI_CreateParam *pPsiParam) 
{
	...
	/* 发送PSI包 */
	{
		...
		lOutPsiInsertPara.m_Interval = PSI_INSERT_INTERVAL;
		lOutPsiInsertPara.m_SdtPacketNum = 1;
		lOutPsiInsertPara.m_SecondSdtPacketPosition = 0;
